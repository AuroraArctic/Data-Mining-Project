...
sparse.save_npz("yourmatrix.npz", te_ary)
your_matrix_back = sparse.load_npz("yourmatrix.npz")

fi = pd.DataFrame.sparse.from_spmatrix(your_matrix_back, columns=te.columns_)
...
timings['Apriori'] = time.time() - start_time
frequent_itemsets
...
timings['Efficient Apriori'] = time.time() - start_time
...
start_time = time.time()
fp_result = fpgrowth(fi, min_support=0.01,use_colnames=True)
fpgrowth(fi, min_support=0.01,use_colnames=True)
timings['FP-Growth'] = time.time() - start_time
